	  ...
	  <View style={styles.container}>
		  <View>
			  <TouchableOpacity onPress={showDatePicker}
			                    className={"bg-indigo-400 p-3 mb-1 rounded-md flex-row items-center"}>
				  <Icon source={Images.calendarBlue} size={25} />
				  <Text
					className={"ml-2 text-white"}>{date ? "Date : " + Intl.DateTimeFormat("fr-FR", options).format(date).charAt(0).toUpperCase() + Intl.DateTimeFormat("fr-FR", options).format(date).slice(1) : t("common.select_date")}</Text>
			  </TouchableOpacity>
		  ...
		  <View>
			  <TouchableOpacity disabled={dateIsSet} onPress={showTimePicker}
			                    className={"bg-orange-400 p-3 rounded-md flex-row items-center"}>
