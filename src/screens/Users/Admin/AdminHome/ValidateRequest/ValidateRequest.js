...
const ValidateRequest = () => {
	...
	async function acceptRequest() {
		try {
			...
			const response = await axios.patch(`${BASE_URL}/api/user/update?id=${phoneNum}`, { "status": "VALIDATED" }, { headers: { Authorization: `Bearer ${token}` } })
	...
	async function rejectRequest() {
		try {
			...
			phoneNum = phoneNum.slice(1)
			phoneNum = "%2B" + phoneNum
			const response = await axios.patch(`${BASE_URL}/api/user/update?id=${phoneNum}`, { "status": "BANNED" }, { headers: { Authorization: `Bearer ${token}` } })
			console.log(response.status)
			if (response.status === 200) {
				navigation.goBack()
			}
		...
		<ScrollView style={{ width: "100%" }}>
			<View style={{ flexDirection: "row", width: "100%" }}>
				...
				<View style={{ marginLeft: 10, width: "65%" }}>
					<TextInput
					  ...
					  editable={false}
					  style={{
						  backgroundColor: "#f0f0f0", height: 60, borderRadius: 18, padding: 10, color: "black",
					  }}
					/>
					<View style={{
			...
			<View>
				...
				<ScrollView horizontal={true} style={{ width: "100%" }}>
					<FullScreenImg preivewWH={screenWidth - 80} img={data.cni_back} />
				</ScrollView>
				<ScrollView horizontal={true} style={{ width: "100%" }}>
					<FullScreenImg preivewWH={screenWidth - 80} img={data.cni_front} />
				</ScrollView>
...
const styles = StyleSheet.create({
	root: {
		backgroundColor: "white", padding: 30, width: "100%", height: "100%", marginTop: 10,
	}, title: {
		marginBottom: 10, marginTop: 10, fontSize: 28,
	}, link: {
		color: "#FE881B", marginTop: 10,
	}, body: {
		padding: 20,
	}, passwordValidation: {
